...
If someone doubts a validator's candidacy as block proposer on a given slot, they can just check the proof and make sure. If it doesn't match, they can report the validator for wrongdoing and have them punished. This will be done by fishermen as part of the regular relay chain block checking, as an illegal roll will result in an invalid relay chain block.
...
Additionally, in Polkadot there's the penalty for illegal randomness rolls which can be detected by fishermen during a regular relay chain block check. An illegal randomness roll will have a validator generate an invalid relay chain block. See Randomness section above for details.
...
2. That smart contract chain has to communicate with other chains to be more useful than a simple non-Polkadot chain, otherwise you're working in a silo not unlike many others out there.
